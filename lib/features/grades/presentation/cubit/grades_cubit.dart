...
class GradesCubit extends Cubit<GradesState> {
  ...
  Future<bool> addMark({
    ...
    try {
      ...
      // GradeItem.fromApiResponse بتشتق grade/section بطريقة مختلفة عن
      // fetchMarks() (اعتمادًا على section.name القادم من رد السيرفر)،
      // وممكن ترجع قيم لا تطابق state.selectedGrade/selectedSection
      // الحاليين حرفيًا. لو صار هيك، visibleGrades (اللي بتفلتر بالتساوي
      // النصي) بتستبعد العلامة المُضافة حديثًا فورًا، فتختفي من
      // الواجهة لحد ما تعمل تحديث كامل يجيب البيانات من fetchMarks()
      // اللي بتحسبها صح. بما إنه الإضافة دايمًا عم تصير جوا الصف/الشعبة
      // المختارين حاليًا، منستخدم هاد القيم مباشرة بدل ما نثق برد
      // السيرفر — متل ما already بيصير بالضبط بـ updateGradeScore.
      final createdForDisplay = created.copyWith(
        grade: state.selectedGrade,
        section: state.selectedSection,
      );

      final updatedItems = [...state.gradeItems, createdForDisplay];
      _safeEmit(state.copyWith(
        gradeItems: updatedItems,
        isLoading: false,
